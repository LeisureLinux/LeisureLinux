...
curl_info() {
    proxy="socks5://127.0.0.1:2019/"
    local org="$(curl -sS -x $proxy ipinfo.io/org)"
    local city="$(curl -sS -x $proxy ipinfo.io/city)"
    local country="$(curl -sS -x $proxy ipinfo.io/country)"
    local region="$(curl -sS -x $proxy ipinfo.io/region)"
